...
import { accountsPayableAgeing, AGEING_BUCKET_LABELS } from "@/lib/accounting/financial-reports";
...
export default async function ApAgeingPage({
  ...
  const rows = await accountsPayableAgeing(asOfDate);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AP Ageing"
        subtitle="Every supplier with an open purchase invoice, bucketed by days since the invoice date — PurchaseInvoice has no due-date field, so this ages from the invoice date rather than an assumed payment term."
      />
