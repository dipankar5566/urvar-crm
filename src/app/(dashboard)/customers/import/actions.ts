...
import { insertCustomerRecord } from "@/app/(dashboard)/customers/actions";
...
function assertImportRole(role: string) {
  if (!canBulkImport(role as never)) {
    throw new Error("Only Super Admin or Sales Manager can bulk-import customers.");
  }
}
...
async function evaluateRows(
...
): Promise<{ verdicts: RowVerdict[]; readyData: Map<number, CustomerFormValues> }> {
  ...
  const readyData = new Map<number, CustomerFormValues>();
...
> {
  ...
  for (const verdict of verdicts) {
    ...
    if (!data) continue;
    try {
      await insertCustomerRecord(data, null, user.id);
      createdCount++;
    } catch {
      errors.push({ rowIndex: verdict.rowIndex, reason: "Could not create this customer." });
    }
  }
