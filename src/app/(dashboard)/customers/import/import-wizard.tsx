...
const DEALER_TIER_OPTIONS = Object.values(DealerTier);
...
export function ImportWizard() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/customers/import/parse", { method: "POST", body: formData });
  ...
  const dealerTierHeader = columnMapping.dealerTier;
  ...
  const distinctDealerTierValues = useMemo(
    () => (dealerTierHeader ? distinctValues(rawRows, dealerTierHeader) : []),
    [rawRows, dealerTierHeader],
  );
  ...
  function proceedToValues() {
    setValueMapping((prev) => {
      ...
      const dealerTier = { ...prev.dealerTier };
      for (const v of distinctDealerTierValues) {
        if (!dealerTier[v]) {
          const guess = guessDealerTier(v);
          if (guess) dealerTier[v] = guess;
        }
      }
      return { customerType, dealerTier };
    });
    setStep("values");
  }
  ...
  function reset() {
    ...
    setFileName("");
    setColumnMapping({});
    setValueMapping({ customerType: {}, dealerTier: {} });
    setVerdicts([]);
    setSummary(null);
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {step === "upload" && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Upload an Excel (.xlsx) or CSV file with your existing customer contacts.
              You&apos;ll map your columns to CRM fields on the next screen.
            </p>
            <Button
              variant="outline"
              size="sm"
              render={<a href="/api/customers/import/template" />}
            >
        ...
        {step === "values" && (
          <div className="space-y-4">
            {!customerTypeHeader && !dealerTierHeader && (
              <p className="text-sm text-muted-foreground">
                You didn&apos;t map Customer Type or Dealer Tier — that&apos;s fine, unmapped rows
                will default to B2C Farmer with no dealer tier. Click back to map them if you do
                have this data.
              </p>
            )}
            ...
            {distinctDealerTierValues.length > 0 && (
              <>
                <p className="pt-2 text-sm text-muted-foreground">
                  Dealer Tier values found — map each one (optional):
                </p>
                <div className="space-y-2">
                  {distinctDealerTierValues.map((v) => (
                    <div key={v} className="flex items-center justify-between gap-3">
                      <span className="text-sm">{v}</span>
                      <Select
                        value={valueMapping.dealerTier[v] || "NONE"}
                        onValueChange={(val) =>
                          setValueMapping((prev) => ({
                            ...prev,
                            dealerTier: {
                              ...prev.dealerTier,
                              [v]: val === "NONE" ? "" : (val as string),
                            },
                          }))
                        }
                      >
            ...
            {unmappedCustomerTypeValues.length > 0 && (
              <p className="text-sm text-destructive">
                Map every Customer Type value before continuing:{" "}
                {unmappedCustomerTypeValues.join(", ")}
              </p>
            )}
            <div className="flex items-center justify-between">
              ...
              <Button
                disabled={unmappedCustomerTypeValues.length > 0 || isPending}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button disabled={readyCount === 0 || isPending} onClick={confirmImport}>
                {isPending ? "Importing…" : `Import ${readyCount} customers`}
              </Button>
            </div>
          </div>
        )}
        ...
        {step === "done" && summary && (
          <div className="space-y-4">
            <p className="text-sm">
              Created <strong>{summary.createdCount}</strong> new customer
